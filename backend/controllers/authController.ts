...
export async function register(req: Request, res: Response): Promise<void> {
  try {
    ...
    // Forzar que solo se pueda registrar como Estudiante
    const finalRole = 'Estudiante';

    // Validar que el grado esté en el rango válido (1-10) si se proporciona
    if (grado !== null && grado !== undefined) {
      const gradoNum = parseInt(grado, 10);
      if (isNaN(gradoNum) || gradoNum < 1 || gradoNum > 10) {
        sendError(res, 'El grado debe ser un número entre 1 y 10', 400);
        return;
      }
    }
